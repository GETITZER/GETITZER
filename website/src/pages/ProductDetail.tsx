              ...
              <table className="w-full text-sm">
                <tbody>
                  {product.specs.map((spec, i) => (
                    <tr
                      key={spec.label}
                      className={`flex sm:table-row ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}
                    >
                      <td className="w-40 sm:w-48 px-4 py-3 font-semibold text-slate-600 shrink-0 border-b border-slate-100">
                        {spec.label}
                      </td>
                      <td className="px-4 py-3 text-slate-800 border-b border-slate-100 flex-1">
                        {spec.value}
                      </td>
                    </tr>
                  ))}
          ...
          <div className="mb-10">
            ...
            <div className="flex flex-wrap gap-3">
              {product.industries.map(ind => (
                <span
                  key={ind}
                  className="flex items-center gap-1.5 text-sm font-medium text-brand-700 bg-brand-50 border border-brand-200 px-3 py-1.5 rounded-full"
                >
          ...
          <div className="mb-10">
            ...
            <div className="flex flex-wrap gap-2">
              {product.compliance.map(c => (
                <span key={c} className="text-sm font-semibold text-slate-700 bg-slate-100 px-3 py-1.5 rounded-lg">
                  {c}
                </span>
              ))}
            </div>
          ...
          <div className="mb-10 border border-dashed border-isa-300 bg-isa-50/50 rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Generate a technical application guide for this valve using Claude AI</p>
              </div>
            </div>
        ...
        <aside className="space-y-5">
          ...
          <div className="card p-5 sticky top-24">
            ...
            <div className="mt-5 pt-4 border-t border-slate-100 space-y-2.5">
              ...
              <p className="text-xs text-center text-slate-400">
                Or use the AI chat button below for instant valve selection guidance
              </p>
            </div>
          </div>
          ...
          <div className="card p-5 bg-slate-50">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              All {product.name.toLowerCase()}s are tested to hydrostatic and pneumatic standards at 1.5× rated pressure before delivery.
            </p>
          </div>
        </aside>
      </div>
